...
##################################################################################################
####################### HOW TO RUN THIS FILE: > `bash -x -v add-bridge.sh` #######################
##################################################################################################
...
firewall-cmd --direct --add-rule ipv4 filter FORWARD 0 -i ${VPN_INTERFACE} -o ${ETH_INTERFACE} -j ACCEPT
firewall-cmd --direct --add-rule ipv4 nat POSTROUTING 0 -o ${ETH_INTERFACE} -j MASQUERADE
