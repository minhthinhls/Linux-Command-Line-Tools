...
resource "google_compute_instance" "bastion-machines" {
    ...
    boot_disk {
        ...
        device_name = "${upper(var.disk_options.type)}-Bastion-Machine-${format("%02d", (count.index + local.offset) + 1)}"
...
resource "google_compute_instance" "private-bastion-machines" {
    ...
    boot_disk {
        auto_delete = false # Must not delete Boot Disk when Instance got terminated.
        device_name = "${upper(var.disk_options.type)}-Bastion-Machine-${format("%02d", (count.index + local.offset + local.expose) + 1)}"
        source = google_compute_disk.bastion-machines[count.index + local.expose].name
    }
