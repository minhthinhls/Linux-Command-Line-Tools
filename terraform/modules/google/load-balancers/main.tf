...
resource "google_compute_address" "load-balancers" {
    ...
    description     = "External-static-ipv4-for-Load-Balancer-${format("%02d", (count.index + local.offset) + 1)}"
...
resource "google_compute_disk" "load-balancers" {
    ...
    name        = "${var.disk_options.type}-load-balancer-${format("%02d", (count.index + local.offset) + 1)}"
