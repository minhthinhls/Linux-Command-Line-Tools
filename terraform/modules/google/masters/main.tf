...
resource "google_compute_address" "masters" {
    ...
    description     = "External-static-ipv4-for-Master-${format("%02d", (count.index + local.offset) + 1)}"
...
resource "google_compute_disk" "masters" {
    ...
    name        = "${var.disk_options.type}-master-${format("%02d", (count.index + local.offset) + 1)}"
...
resource "google_compute_instance" "private-masters" {
    ...
    boot_disk {
        ...
        source = google_compute_disk.masters[count.index + local.expose].name
    }
