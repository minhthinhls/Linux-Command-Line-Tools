...
resource "google_compute_address" "snapshot-masters" {
    ...
    name            = "snapshot-master-${format("%02d", (count.index + local.offset) + 1)}"
    address_type    = "EXTERNAL" # ["INTERNAL", "EXTERNAL"]
    description     = "External-static-ipv4-for-Snapshot-Master-${format("%02d", (count.index + local.offset) + 1)}"
...
resource "google_compute_disk" "snapshot-masters" {
    ...
    name        = "${var.disk_options.type}-snapshot-master-${format("%02d", (count.index + local.offset) + 1)}"
...
# @command >> terraform state rm google_compute_snapshot.masters;
# ----------------------------------------------------------------------------------------------------------------------------------------------------
resource "google_compute_snapshot" "masters" {
    count       = try(var.snapshots_instances, 0) # Switch to [1] to provision again.
    name        = var.snapshot_options.name
    zone        = var.zone
    labels      = tomap({role = "master"})
    source_disk = google_compute_disk.snapshot-masters[count.index].id
    description = "Snapshot ${upper(var.disk_options.type)} for Master Nodes."
...
resource "google_compute_instance" "snapshot-masters" {
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------
    name         = "master-${format("%02d", (count.index + local.offset) + 1)}"
    hostname     = "master-${format("%02d", (count.index + local.offset) + 1)}.e8s.io"
    ...
    metadata = {
        ...
        startup-script = <<EOF
            sudo hostnamectl set-hostname 'master-${format("%02d", (count.index + local.offset) + 1)}.e8s.io';
            echo 'master-${format("%02d", (count.index + local.offset) + 1)}.e8s.io' | sudo tee /etc/hostname > /dev/null;
    ...
    boot_disk {
        ...
        device_name = "${upper(var.disk_options.type)}-Master-${format("%02d", (count.index + local.offset) + 1)}"
...
# @see {@link https://github.com/hashicorp/terraform/issues/11770}
# ----------------------------------------------------------------------------------------------------------------------------------------------------
resource "google_compute_instance" "private-snapshot-masters" {
    count        = ((var.number_instances > var.reserved_external_ips) ?
    ...
    # @see {@link https://stackoverflow.com/questions/49841511/change-hostname-permanently-in-google-compute-engine-instance-after-reboot/}
    # ------------------------------------------------------------------------------------------------------------------------------------------------
    name         = "master-${format("%02d", (count.index + local.offset + local.expose) + 1)}"
    hostname     = "master-${format("%02d", (count.index + local.offset + local.expose) + 1)}.e8s.io"
    machine_type = var.gce_options.machine_type # [["e2-standard-4"] -> ["4CPUs :: 16GBs RAM"]] && [["e2-standard-8"] -> ["8CPUs :: 32GBs RAM"]]
    zone         = var.zone
    tags         = ["masters"]
    ...
    metadata = {
        ssh-keys = "${var.secrets.gce_ssh_user}:${file(var.secrets.gce_ssh_pub_key_file)}"
        startup-script = <<EOF
            sudo hostnamectl set-hostname 'master-${format("%02d", (count.index + local.offset + local.expose) + 1)}.e8s.io';
            echo 'master-${format("%02d", (count.index + local.offset + local.expose) + 1)}.e8s.io' | sudo tee /etc/hostname > /dev/null;
            sudo cp --force /root/.bashrc /home/admin.e8s.io/.bashrc 2> /dev/null;
            sudo systemctl restart kubelet 2> /dev/null;
        EOF
    }
    ...
    boot_disk {
        auto_delete = false # Must not delete Boot Disk when Instance got terminated.
        device_name = "${upper(var.disk_options.type)}-Master-${format("%02d", (count.index + local.offset + local.expose) + 1)}"
        source = google_compute_disk.snapshot-masters[count.index + local.expose].name
    }
