...
resource "google_compute_address" "snapshot-workers" {
    ...
    name            = "snapshot-worker-${format("%02d", (count.index + local.offset) + 1)}"
    address_type    = "EXTERNAL" # ["INTERNAL", "EXTERNAL"]
    description     = "External-static-ipv4-for-Snapshot-Worker-${format("%02d", (count.index + local.offset) + 1)}"
...
resource "google_compute_disk" "snapshot-workers" {
    ...
    name        = "${var.disk_options.type}-snapshot-worker-${format("%02d", (count.index + local.offset) + 1)}"
...
resource "google_compute_snapshot" "workers" {
    ...
    zone        = var.zone
    labels      = tomap({role = "worker"})
    source_disk = google_compute_disk.snapshot-workers[count.index].id
    description = "Snapshot ${upper(var.disk_options.type)} for Worker Nodes."
...
# @see {@link https://github.com/hashicorp/terraform/issues/11770}
# ----------------------------------------------------------------------------------------------------------------------------------------------------
resource "google_compute_instance" "private-snapshot-workers" {
    count        = ((var.number_instances > var.reserved_external_ips) ?
    ...
    boot_disk {
        ...
        source = google_compute_disk.snapshot-workers[count.index + local.expose].name
    }
