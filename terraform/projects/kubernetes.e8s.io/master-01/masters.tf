...
# ----------------------------------------------------------------------------------------------------------------------------------------------------
locals {
    offset = var.master_instances.offset_instances # Instance Indexing Offset.
    expose = var.master_instances.reserved_external_ips # Instance Public Offset.
}
...
resource "google_compute_address" "masters" {
    ...
    name            = "master-0${(count.index + local.offset) + 1}"
    address_type    = "EXTERNAL" # ["INTERNAL", "EXTERNAL"]
    description     = "External-static-ipv4-for-Master-0${(count.index + local.offset) + 1}"
...
resource "google_compute_disk" "masters" {
    ...
    name        = "ssd-master-0${(count.index + local.offset) + 1}"
...
resource "google_compute_instance" "masters" {
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------
    name         = "master-0${(count.index + local.offset) + 1}"
    hostname     = "master-0${(count.index + local.offset) + 1}.e8s.io"
    ...
    boot_disk {
        ...
        device_name = "SSD-Master-0${(count.index + local.offset) + 1}"
    ...
    network_interface {
        ...
        network_ip  = "172.16.1.${(100 + count.index + local.offset) + 1}"
        access_config {
...
resource "google_compute_instance" "private-masters" {
    ...
    # @see {@link https://stackoverflow.com/questions/49841511/change-hostname-permanently-in-google-compute-engine-instance-after-reboot/}
    # ------------------------------------------------------------------------------------------------------------------------------------------------
    name         = "master-0${(count.index + local.offset + local.expose) + 1}"
    hostname     = "master-0${(count.index + local.offset + local.expose) + 1}.e8s.io"
    machine_type = "e2-highmem-2" # [["e2-small"], ["e2-custom-2-3072"]] -> [["2CPUs :: 2GBs RAM"], ["2CPUs :: 3GBs RAM"]]
    tags         = ["masters"]
    ...
    boot_disk {
        auto_delete = false # Must not delete Boot Disk when Instance got terminated.
        device_name = "SSD-Master-0${(count.index + local.offset + local.expose) + 1}"
    ...
    network_interface {
        ...
        network     = google_compute_network.global_vpc.self_link
        subnetwork  = google_compute_subnetwork.hongkong.self_link
        network_ip  = "172.16.1.${(100 + count.index + local.offset + local.expose) + 1}"
    }
