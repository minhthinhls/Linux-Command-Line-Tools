...
resource "google_compute_snapshot" "masters" {
    count       = var.snapshot_master_instances.snapshots_instances # Switch to [1] to provision again.
    name        = "snapshot-masters"
    zone        = "asia-east2-a" # "${var.zone}"
    source_disk = google_compute_disk.snapshot-masters[count.index].id
    description = "Snap shot SSD for Master Nodes."
    labels = {
        role = "master"
    }
...
resource "google_compute_instance" "snapshot-masters" {
    ...
    network_interface {
        ...
        network_ip  = "172.16.1.${(100 + count.index)}"
        access_config {
