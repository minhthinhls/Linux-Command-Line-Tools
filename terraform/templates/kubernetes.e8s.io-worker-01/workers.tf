...
resource "google_compute_address" "workers" {
    ...
    name            = "worker-0${(count.index + local.offset) + 1}"
    address_type    = "EXTERNAL" # ["INTERNAL", "EXTERNAL"]
    description     = "External-static-ipv4-for-Worker-0${(count.index + local.offset) + 1}"
...
resource "google_compute_disk" "workers" {
    ...
    name        = "ssd-worker-0${(count.index + local.offset) + 1}"
...
resource "google_compute_instance" "workers" {
    ...
    # ------------------------------------------------------------------------------------------------------------------------------------------------
    name         = "worker-0${(count.index + local.offset) + 1}"
    hostname     = "worker-0${(count.index + local.offset) + 1}.e8s.io"
    ...
    metadata = {
        ...
        startup-script = <<EOF
            sudo hostnamectl set-hostname 'worker-0${(count.index + local.offset) + 1}.e8s.io';
            echo 'worker-0${(count.index + local.offset) + 1}.e8s.io' | sudo tee /etc/hostname > /dev/null;
        EOF
    }
    ...
    boot_disk {
        ...
        device_name = "SSD-Worker-0${(count.index + local.offset) + 1}"
    ...
    network_interface {
        ...
        network_ip  = "172.16.2.${(100 + count.index + local.offset) + 1}"
        access_config {
...
resource "google_compute_instance" "private-workers" {
    ...
    # @see {@link https://stackoverflow.com/questions/49841511/change-hostname-permanently-in-google-compute-engine-instance-after-reboot/}
    # ------------------------------------------------------------------------------------------------------------------------------------------------
    name         = "worker-0${(count.index + local.offset + local.expose) + 1}"
    hostname     = "worker-0${(count.index + local.offset + local.expose) + 1}.e8s.io"
    machine_type = "e2-highmem-2" # [["e2-small"], ["e2-custom-2-3072"]] -> [["2CPUs :: 2GBs RAM"], ["2CPUs :: 3GBs RAM"]]
    tags         = ["workers"]
    ...
    metadata = {
        ssh-keys = "${var.gce_ssh_user}:${file(var.gce_ssh_pub_key_file)}"
        startup-script = <<EOF
            sudo hostnamectl set-hostname 'worker-0${(count.index + local.offset + local.expose) + 1}.e8s.io';
            echo 'worker-0${(count.index + local.offset + local.expose) + 1}.e8s.io' | sudo tee /etc/hostname > /dev/null;
        EOF
    }
    ...
    boot_disk {
        auto_delete = false # Must not delete Boot Disk when Instance got terminated.
        device_name = "SSD-Worker-0${(count.index + local.offset + local.expose) + 1}"
    ...
    network_interface {
        ...
        network     = google_compute_network.global_vpc.self_link
        subnetwork  = google_compute_subnetwork.hongkong.self_link
        network_ip  = "172.16.2.${(100 + count.index + local.offset + local.expose) + 1}"
    }
